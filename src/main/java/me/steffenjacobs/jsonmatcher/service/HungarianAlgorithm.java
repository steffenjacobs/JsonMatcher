 ...
 *
 *
 * @author https://github.com/aalmi | march 2014, modified by Steffen Jacobs
 * @version 1.0
 */
public class HungarianAlgorithm {
	...
	private final double[][] matrix; // initial matrix (cost matrix)
	...
	public int[][] findOptimalAssignment() {
		...
		while (!allColumnsAreCovered()) {
			...
			if (squareInRow[mainZero[0]] == -1) {
				...
				colIsCovered[squareInRow[mainZero[0]]] = 0; // uncover column of
															// mainZero
				step7();
			}
		}
	...
	/**
	 * Step 1: Reduce the matrix so that in each row and column at least one
	 * zero exists: 1. subtract each row minima from each element of the row 2.
	 * subtract each column minima from each element of the column
	 */
	private void step1() {
		// rows
	...
	/**
	 * Step 2: mark each 0 with a "square", if there are no other marked zeroes
	...
	private void step2() {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < matrix.length; j++) {
				// mark if current value == 0 & there are no other marked zeroes
				// in the same row or column
	...
	/**
	 * Step 7: 1. Find the smallest uncovered value in the matrix. 2. Subtract
	 * it from all uncovered values 3. Add it to all twice-covered values
	 */
	private void step7() {
	...
	private void step6(int[] mainZero) {
		...
		do {
			// (b)
			// add Z_1 to K if
			// there is a zero Z_1 which is marked with a "square " in the
			// column of Z_0
			if (squareInCol[j] != -1) {
			...
			// if no zero element Z_1 marked with "square" exists in the column
			// of Z_0, then cancel the loop
			if (!found) {
				break;
			}
